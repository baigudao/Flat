...
public class DeviceUtil {
	...
	public static String getDeviceId(Context context) {
		...
		if (null != imei && !SPECIAL_IMEI.equals(imei)) {
			Logger.d(Thread.currentThread().getName(), "imei:" + imei);
			return imei;
		}else {
			...
			if (null != macAddress){
				Logger.d(Thread.currentThread().getName(), "mac:" + macAddress);
				return macAddress.replace(".", "").replace(":", "").replace("-", "").replace("_", "").replace("+", "").replace("-", "").replace("=", "");
			}else {
				String deviceId = Secure.getString(context.getContentResolver(), Secure.ANDROID_ID);
				if (null != deviceId) {
					Logger.d(Thread.currentThread().getName(), "ANDROID_ID:" + deviceId);
