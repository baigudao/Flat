...
//    private static ExecutorService exec;
     ...
     */
//    public static void init(@Type final int type, final int corePoolSize) {
//        // 构造有定时功能的线程池
...
//    }
     ...
     * @param corePoolSize 只对Fixed和Scheduled线程池起效
     */
    public static void init( final int corePoolSize) {
        // 构造有定时功能的线程池
        // ThreadPoolExecutor(corePoolSize, Integer.MAX_VALUE, 10L, TimeUnit.MILLISECONDS, new BlockingQueue<Runnable>)
        scheduleExec = Executors.newScheduledThreadPool(corePoolSize);
        exec = new ThreadPoolExecutor(corePoolSize,corePoolSize,60L,TimeUnit.SECONDS,new LinkedBlockingDeque<>());
    }
    public static int getLargestPoolSize(){
        return exec.getLargestPoolSize();
    }
    /**
     * 在未来某个时间执行给定的命令
