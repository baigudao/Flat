...
import com.taisau.flat.util.NetworkUtil;
...
public class NetClient {
    ...
    private static CompareAPI compareAPI;

    public CompareAPI getCompareAPI() {
        return compareAPI == null ? configRetrofit(CompareAPI.class, false) : compareAPI;
    }

    private <T> T configRetrofit(Class<T> service, boolean isGetToken) throws IllegalArgumentException {
        Retrofit retrofit = new Retrofit.Builder().baseUrl(Preference.getCompareServerUrl())
                .client(configClient())
...
//    final static HashMap<String, List<Cookie>> cookieStore = new HashMap<>();
   ...
   /*     //为所有请求添加头部 Header 配置的拦截器
       ...
       /* if (true) {
        ...
        //设置缓存路径
        File httpCacheDirectory = new File(FileUtils.getExternalCacheDir(FlatApplication.getApplication()), "http-cache");
...
//        okHttpClient.cookieJar(new CookiesManager());
    ...
    public Interceptor getInterceptor() {
        return new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                Request request = chain.request();
                if (!NetworkUtil.isConnected(FlatApplication.getApplication())) {
